...
from Utils.Extended_data import DataLoader_GPU, My_Dataset, DataLoader_CPU
from Utils.Sysmdl_Parameter import Model_3DCT
from Pipline_IF import Pipeline_IF
from IFNet_nn import Decentralized_IFNetNN
from datetime import datetime
from Extended_DIF_test import Extended_DIFTest
from CEKF_test import CEKFTest
...
def main(args):
    ...
    global_state_idx = [0, 2, 4]
    global_velocity_idx = [1, 3, 5]

    position_idx = [[0, 2], [0, 2], [0, 2, 4], [0]]
    velocity_idx = [[1, 3], [1, 3], [1, 3, 5], [1]]

    initial_state_all_training = torch.ones([model.N_E, 6, 1]) * 150
    initial_state_all_validation = torch.ones([model.N_CV, 6, 1]) * 150
    initial_state_all_test = torch.ones([model.N_T, 6, 1]) * 150
    for index in global_velocity_idx:
        initial_state_all_training[:, index, :] = 100
        initial_state_all_validation[:, index, :] = 100
        initial_state_all_test[:, index, :] = 95
    initial_state_all = {
    ...
    sys_model_exact.InitSequence(model.x0, model.P0, initial_state_all)
    ...
    sys_model_inexact.InitSequence(model.x0, model.P0, initial_state_all)
    ...
    sys_model_cekf.InitSequence(model.x0, model.P0, initial_state_all)
    ...
    # plt.figure()
    # plt.plot(test_target[0, 0, :].cpu().numpy(), test_target[0, 2, :].cpu().numpy())
    # plt.show()
    ...
    IFNet_Pipeline = Pipeline_IF(strTime, modelFolder, "3DCT")
    IFNet_Pipeline.setssModel(sys_model_exact, sys_model_ekf_inexact, args.batch_size)
    IFNet_model = Decentralized_IFNetNN()
    IFNet_model.Build(sys_model_exact, sys_model_ekf_inexact, args.batch_size)
    ...
    DIFNet_time = DIFNet_time * args.batch_size
    ...
    ax4.legend()
    plt.savefig("Nolinear_s4_pos_rmse.pdf", bbox_inches="tight", dpi=300, format="pdf")
    ...
    plt.savefig("Nolinear_s4_vel_rmse.pdf", bbox_inches="tight", dpi=300, format="pdf")

    plt.show()

    print(
        torch.tensor([DIF_exact_time, DIF_inexact_time, CEKF_time, DIFNet_time])
        / DIF_exact_time
    )
